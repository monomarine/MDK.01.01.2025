...
namespace Graphs
{
    internal class GraphByList
    {
        ...
        public double AverageGPA()
        {
            ...
            double sum = allNodes.Sum(n => n.Value.GPA);
            return sum / allNodes.Count;
        }
        public Node MostSocialStudent()
        {
            var allNodes = GetAllNodes();
            return allNodes.OrderByDescending(n => n.Neighbors.Count).FirstOrDefault();
        }

        //  метод для получения всех узлов графа
        ...
        {
            if (current == null || vector.Contains(current)) return;
            vector.Add(current);
            list.Add(current);
